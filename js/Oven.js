...
export default class Oven extends EntityInteractable {
    constructor(x, y, width, height, station, engine) {
        ...
        this.prompt = new OnScreenTextSystem(this,
            this.x + (width / 2), this.y - (height / 4), "Press E to Cook", false);
...
export class Fryer extends EntityInteractable {
constructor(x, y, width, height, station, engine) {
        super();
        this.x = x;
        this.y = y;
        ...
        this.toggleState = false;
        this.prompt = new OnScreenTextSystem(this,
            this.x + (width / 2), this.y - (height / 4), "Press E to Fry", false);
        this.timer = new OnScreenTextSystem(this,this.x + (width / 2), this.y - (height / 4), "0:0" + Math.ceil((this.cookingTime - this.elapsedCook.toString(10) / 60), false));
        engine.addEntity(this.prompt);
        engine.addEntity(this.timer);

        this.displayingUI = false;
        this.sprite = new Animator(ASSET_MANAGER.getAsset("/Assets/Entities/Fryer.png"), 0, 0, 16, 32, 1, 1, 0, false, false);
    }
    ...
    interact(player) {
        if (!this.toggleable) return;
        if (this.station.canHandleStep(STEP_TYPE.FRY)) {
                this.toggleable = false;
                this.toggleState = true;
                this.isCooking = true;
                this.elapsedCook = 0;
                ...
                this.station.beginStep(STEP_TYPE.FRY);
                this.color = cookColor;
